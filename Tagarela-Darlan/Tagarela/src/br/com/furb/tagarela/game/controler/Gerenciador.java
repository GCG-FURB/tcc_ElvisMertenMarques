...
//			planoDAO.deleteAll();	 
			...
			for (Plano plano: planos) {				
				...
				for (Prancha prancha : plano.getPranchas()) {					
					...
					pranchaXSimboloBD.setServerID((long)getNextServerID(pranchaXSimboloDAO));
					pranchaXSimboloBD.setPlanServerID((long)pranchaBD.getServerID());
					pranchaXSimboloBD.setSymbolServerID((long)simboloBD.getServerID());
...
//			simboloDAO;
			...
			for (SymbolPlan pranchaXSimbolo : pranchaXSimboloDAO.loadAll()) {
				Log.i("PRANCHAS X SIMBOLOS", pranchaXSimbolo.getServerID() + " " + pranchaXSimbolo.getPlanServerID() + " " + pranchaXSimbolo.getSymbolServerID());				
			}
			Log.i("PRANCHAS X SIMBOLOS", "-------------------------------");
			...
			for (GroupPlanRelationship planoXPranchaBD : planoXPranchaDAO.queryRaw("where group_ID = ?", planoBD.getServerID().toString())) {
				...
				for (Plan pranchaBD : pranchaDAO.queryRaw("where server_ID = ?", planoXPranchaBD.getPlanID().toString())) {
					...
					for (SymbolPlan pranchaXSimboloBD : pranchaXSimboloDAO.queryRaw("where plan_ID = ?", pranchaBD.getServerID().toString())) {
						...
						for (Symbol simboloBD : simboloDAO.queryRaw("where server_ID = ?", pranchaXSimboloBD.getSymbolServerID().toString())) {
